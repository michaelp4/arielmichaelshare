...
edata=ones(1,length(ydata))*0.1; % errors in y-data points
xErr = xerr;
dd=1./(edata.*edata);
...
i=1;
for xe = xErr.'
   edata(i) = edata(i) + slope.Estimate*2*xe;
   i = i + 1;
end
...
title('Voltage vs Radius Squared','Interpreter','latex');
ylabel('Voltage [V]');
xlabel('Radius squared [m^2]');    
% addTrap(x,y,3,'-');
% addTrap(x,y,6,'r');
% addTrap(x,y,18,'y')
